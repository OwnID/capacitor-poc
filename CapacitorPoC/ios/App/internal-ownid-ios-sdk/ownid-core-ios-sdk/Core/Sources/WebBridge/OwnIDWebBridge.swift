...
extension OwnID.CoreSDK {
    ...
    public class OwnIDWebBridge: NSObject, WKScriptMessageHandler {
                ...
                let allOrigins = configOrigins.union(self.allowedOriginRules)
                    ...
                    .compactMap { url in
                        ...
                        case "https" where url.scheme?.caseInsensitiveCompare("https") == .orderedSame:
                            return url
                        case "capacitor" where url.scheme?.caseInsensitiveCompare("capacitor") == .orderedSame:
                            return url
                        default:
                            return nil
                        }
        ...
        private func getJSInterface() -> String {
            let namespaces: [WebNamespace] = [OwnIDWebBridgeFido(), OwnIDWebBridgeFlow(), OwnIDWebBridgeMetadata(), OwnIDWebBridgeStorage()]
